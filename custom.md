...
- Scope: minor tweak.
- Goal: avoid the reserved `openai-curated` marketplace identity while preserving all plugin entries and paths.
...
- Exact changes: new workflow that fetches `openai/plugins`, merges `upstream/main`, only auto-resolves conflicts in the two marketplace JSON files, reapplies the personal marketplace names, validates the manifests, commits if needed, and pushes changed `main`.
